---
name: codeguard-cuda
description: 使用 Codeguard 对 CUDA 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 clang-tidy --quiet '{file}' 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
3. 先只读检查：`bash -c 'find . -name '"'"'*.cu'"'"' | xargs -r clang-tidy --quiet'`。
...
- “检查这个 CUDA 项目，先不要修改，给我 lint 失败分类。”
- “只修复 CUDA 的格式问题，语义问题先列清单。”
- “提交前验证 CUDA lint、测试和构建证据。”
...
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | CUDA Toolkit + clangd |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `clang-tidy --quiet '{file}'` |
| format/fix | `clang-format -i '{file}'` |
| 项目级 gate | `bash -c 'find . -name '"'"'*.cu'"'"' \| xargs -r clang-tidy --quiet'` |
...
运行 `bash -c 'find . -name '"'"'*.cu'"'"' | xargs -r clang-tidy --quiet'`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
7. **领域陷阱** — CUDA 文件同时受主机编译器与 nvcc 约束；clang-tidy 不能替代真实 CUDA 构建。
