...
description: 使用 Codeguard 对 Ansible 项目识别但不冒充已执行代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 ansible-lint 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# Ansible Codeguard 门禁

> 目标：把“看起来没问题”变成可重复的工具证据，并且诚实区分通过、失败、无法验证和 planned。

## 30 秒开始

当前 `planned` 只表示 Codeguard 能识别 无固定扩展名，不表示存在可执行门禁。
...
- 用户要求检查或修复 Ansible 代码规范。
- AI 修改了 无固定扩展名，需要提交前验证。
- CI 出现与 `ansible-lint` 相关的失败。
...
| 项目 | 当前事实 |
|---|---|
| 状态 | `planned`（自 Codeguard V0.3） |
| 文件扩展名 | 无固定扩展名 |
| 项目标记 | `ansible.cfg`, `playbook.yml` |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | playbook 检查由 ansible-lint 承接（yaml 通道）；语义级检查待集成 |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `ansible-lint` |
| format/fix | `ansible-lint --fix` |
| 项目级 gate | `ansible-lint` |
...
运行 `ansible-lint`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `ansible-lint --fix`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
1. **领域陷阱** — 当前条目标记为 planned；不得把 YAML 通道的结果冒充 Ansible 语义门禁。
