...
description: 使用 Codeguard 对 Clojure 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 clj-kondo --lint src 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# Clojure Codeguard 门禁

> 目标：把“看起来没问题”变成可重复的工具证据，并且诚实区分通过、失败、无法验证和 planned。

## 30 秒开始

1. 在仓库根确认 ``deps.edn`, `project.clj`` 或目标文件存在。
2. 探测工具：`clj-kondo --version`。
3. 先只读检查：`clj-kondo --lint src`。
4. 将结果分为：代码违规、配置缺失、工具缺失、工具内部错误。
5. 只对可安全修复项执行：`cljstyle`。
6. 复跑 lint，并执行项目已有测试/构建门禁。

可直接提出：

- “检查这个 Clojure 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Clojure 的格式问题，语义问题先列清单。”
- “提交前验证 Clojure lint、测试和构建证据。”
...
1. 根据 `.clj`, `.cljs`, `.cljc`, `.edn` 与项目标记识别 Clojure 工作区。
2. 执行 `clj-kondo --lint src` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `cljstyle` 处理可逆问题并复扫。
...
- 用户要求检查或修复 Clojure 代码规范。
- AI 修改了 `.clj`, `.cljs`, `.cljc`, `.edn`，需要提交前验证。
- CI 出现与 `clj-kondo --lint src` 相关的失败。
...
| 项目标记 | `deps.edn`, `project.clj` |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | brew install clj-kondo cljstyle |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `clj-kondo --lint src` |
| format/fix | `cljstyle` |
| 项目级 gate | `clj-kondo --lint src` |
...
运行 `clj-kondo --lint src`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `cljstyle`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
7. **领域陷阱** — clj-kondo cache 与 classpath 会影响宏分析；宏项目需提供导出配置。
