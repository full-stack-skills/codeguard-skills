...
description: 使用 Codeguard 对 Erlang 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 elvis rock 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
1. 在仓库根确认 ``rebar.config`` 或目标文件存在。
2. 探测工具：`elvis --version`。
3. 先只读检查：`elvis rock`。
...
- “检查这个 Erlang 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Erlang 的格式问题，语义问题先列清单。”
- “提交前验证 Erlang lint、测试和构建证据。”
...
1. 根据 `.erl`, `.hrl` 与项目标记识别 Erlang 工作区。
2. 执行 `elvis rock` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `erlfmt` 处理可逆问题并复扫。
...
- 用户要求检查或修复 Erlang 代码规范。
- AI 修改了 `.erl`, `.hrl`，需要提交前验证。
- CI 出现与 `elvis rock` 相关的失败。
...
| 文件扩展名 | `.erl`, `.hrl` |
| 项目标记 | `rebar.config` |
| 接入配置 | `elvis.config`, `elvis.config.js` |
| 工具准备 | rebar3 plugins / escript |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `elvis rock` |
| format/fix | `erlfmt` |
| 项目级 gate | `elvis rock` |
...
读取 `elvis.config`, `elvis.config.js` 及 CI 中的实际命令。仓库配置优先于本技能示例；如果两者冲突，先报告差异。
...
运行 `elvis rock`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `erlfmt`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
7. **领域陷阱** — Elvis 与 erlfmt 依赖 rebar 项目上下文；OTP 版本也会改变可用语法。
