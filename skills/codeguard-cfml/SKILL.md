---
name: codeguard-cfml
description: 使用 Codeguard 对 CFML (ColdFusion) 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 cflint '{file}' 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# CFML (ColdFusion) Codeguard 门禁
...
2. 探测工具：`cflint --version`。
3. 先只读检查：`bash -c 'find . \( -name '"'"'*.cfc'"'"' -o -name '"'"'*.cfm'"'"' \) -type f -print0 | xargs -0 -r cflint'`。
...
- “检查这个 CFML (ColdFusion) 项目，先不要修改，给我 lint 失败分类。”
- “只修复 CFML (ColdFusion) 的格式问题，语义问题先列清单。”
- “提交前验证 CFML (ColdFusion) lint、测试和构建证据。”
...
1. 根据 `.cfc`, `.cfm`, `.cfs` 与项目标记识别 CFML (ColdFusion) 工作区。
2. 执行 `cflint '{file}'` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `人工最小修复` 处理可逆问题并复扫。
...
- 用户要求检查或修复 CFML (ColdFusion) 代码规范。
- AI 修改了 `.cfc`, `.cfm`, `.cfs`，需要提交前验证。
- CI 出现与 `cflint '{file}'` 相关的失败。
...
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | brew install cflint |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `cflint '{file}'` |
| format/fix | `未接入` |
| 项目级 gate | `bash -c 'find . \( -name '"'"'*.cfc'"'"' -o -name '"'"'*.cfm'"'"' \) -type f -print0 \| xargs -0 -r cflint'` |
...
运行 `bash -c 'find . \( -name '"'"'*.cfc'"'"' -o -name '"'"'*.cfm'"'"' \) -type f -print0 | xargs -0 -r cflint'`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `人工最小修复`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
7. **领域陷阱** — CFML 方言与引擎差异会影响 CFLint 结果；需注明 Adobe CF 或 Lucee。
