...
description: 使用 Codeguard 对 Protobuf 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 buf lint 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# Protobuf Codeguard 门禁
...
5. 只对可安全修复项执行：`buf format`。
6. 复跑 lint，并执行项目已有测试/构建门禁。

可直接提出：

- “检查这个 Protobuf 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Protobuf 的格式问题，语义问题先列清单。”
- “提交前验证 Protobuf lint、测试和构建证据。”
...
1. 根据 `.proto` 与项目标记识别 Protobuf 工作区。
2. 执行 `buf lint` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `buf format` 处理可逆问题并复扫。
...
- 用户要求检查或修复 Protobuf 代码规范。
- AI 修改了 `.proto`，需要提交前验证。
- CI 出现与 `buf lint` 相关的失败。
...
| 文件扩展名 | `.proto` |
| 项目标记 | 无专用标记文件 |
| 接入配置 | `buf.yaml`, `buf.gen.yaml`, `buf.work.yaml` |
| 工具准备 | brew install bufbuild/buf/buf |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `buf lint` |
| format/fix | `buf format` |
| 项目级 gate | `buf lint` |
...
读取 `buf.yaml`, `buf.gen.yaml`, `buf.work.yaml` 及 CI 中的实际命令。仓库配置优先于本技能示例；如果两者冲突，先报告差异。
...
运行 `buf lint`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `buf format`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
1. **领域陷阱** — Buf lint 通过后仍应检查 breaking changes；风格合规不等于 wire compatibility。
