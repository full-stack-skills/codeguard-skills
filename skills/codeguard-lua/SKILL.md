---
name: codeguard-lua
description: 使用 Codeguard 对 Lua 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 luacheck . 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
- “检查这个 Lua 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Lua 的格式问题，语义问题先列清单。”
- “提交前验证 Lua lint、测试和构建证据。”
...
1. 根据 `.lua` 与项目标记识别 Lua 工作区。
2. 执行 `luacheck .` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `stylua .` 处理可逆问题并复扫。
...
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | luarocks install luacheck; cargo install stylua |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `luacheck .` |
| format/fix | `stylua .` |
| 项目级 gate | `luacheck .` |
...
运行 `luacheck .`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `stylua .`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
1. **领域陷阱** — Lua 版本和宿主环境差异大；Luacheck globals 必须与运行时一致。
