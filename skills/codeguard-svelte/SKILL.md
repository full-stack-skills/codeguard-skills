...
description: 使用 Codeguard 对 Svelte 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 npx --no-install eslint . 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
- “检查这个 Svelte 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Svelte 的格式问题，语义问题先列清单。”
- “提交前验证 Svelte lint、测试和构建证据。”
...
1. 根据 `.svelte` 与项目标记识别 Svelte 工作区。
2. 执行 `npx --no-install eslint .` 并保留退出码、作用域和工具版本。
...
- 用户要求检查或修复 Svelte 代码规范。
- AI 修改了 `.svelte`，需要提交前验证。
- CI 出现与 `npx --no-install eslint .` 相关的失败。
...
运行 `npx --no-install eslint .`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
7. **领域陷阱** — Svelte parser 与 ESLint 主版本需要兼容；解析失败不能降级为跳过。
