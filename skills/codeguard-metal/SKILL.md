...
description: 使用 Codeguard 对 Metal 项目识别但不冒充已执行代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 尚未接入的检查链 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
当前 `planned` 只表示 Codeguard 能识别 `.metal`，不表示存在可执行门禁。
...
- 用户要求检查或修复 Metal 代码规范。
- AI 修改了 `.metal`，需要提交前验证。
- CI 出现与 `Metal 工具链` 相关的失败。
...
1. **领域陷阱** — 当前条目标记为 planned；需要 Xcode/metal 编译链证据后才能升级。
