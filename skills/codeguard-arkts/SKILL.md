...
description: 使用 Codeguard 对 ArkTS (HarmonyOS) 项目识别但不冒充已执行代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 尚未接入的检查链 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# ArkTS (HarmonyOS) Codeguard 门禁

> 目标：把“看起来没问题”变成可重复的工具证据，并且诚实区分通过、失败、无法验证和 planned。

## 30 秒开始

当前 `planned` 只表示 Codeguard 能识别 `.ets`，不表示存在可执行门禁。
...
1. 识别 ArkTS (HarmonyOS) 文件和相关用户意图。
...
- 用户要求检查或修复 ArkTS (HarmonyOS) 代码规范。
- AI 修改了 `.ets`，需要提交前验证。
- CI 出现与 `ArkTS (HarmonyOS) 工具链` 相关的失败。
...
| 项目 | 当前事实 |
|---|---|
| 状态 | `planned`（自 Codeguard V0.5） |
| 文件扩展名 | `.ets` |
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | 无独立 CLI linter（依赖平台 IDE 诊断） |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `未接入` |
| format/fix | `未接入` |
| 项目级 gate | `未接入` |
...
1. **领域陷阱** — 当前条目标记为 planned；需要 HarmonyOS/DevEco 官方诊断证据后才能升级。
