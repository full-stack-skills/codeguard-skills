---
name: codeguard-vue
description: 使用 Codeguard 对 Vue 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 npx --no-install eslint --ext .vue . 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
3. 先只读检查：`npx --no-install eslint --ext .vue .`。
4. 将结果分为：代码违规、配置缺失、工具缺失、工具内部错误。
5. 只对可安全修复项执行：`npx eslint --ext .vue . --fix`。
6. 复跑 lint，并执行项目已有测试/构建门禁。

可直接提出：

- “检查这个 Vue 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Vue 的格式问题，语义问题先列清单。”
- “提交前验证 Vue lint、测试和构建证据。”
...
1. 根据 `.vue` 与项目标记识别 Vue 工作区。
2. 执行 `npx --no-install eslint --ext .vue .` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `npx eslint --ext .vue . --fix` 处理可逆问题并复扫。
...
- 用户要求检查或修复 Vue 代码规范。
- AI 修改了 `.vue`，需要提交前验证。
- CI 出现与 `npx --no-install eslint --ext .vue .` 相关的失败。
...
| 项目 | 当前事实 |
|---|---|
| 状态 | `stable`（自 Codeguard V0.4） |
| 文件扩展名 | `.vue` |
| 项目标记 | 无专用标记文件 |
| 接入配置 | `eslint.config.js`, `eslint.config.mjs`, `eslint.config.cjs`, `eslint.config.ts`, `.eslintrc.json`, `.eslintrc.js`, `.eslintrc.yml`, `.pre-commit-config.yaml`, `.eslintrc`, `.eslintrc.cjs` |
| 工具准备 | eslint-plugin-vue |
| 探测命令 | `npx --no-install eslint --version` |
| lint | `npx --no-install eslint --ext .vue .` |
| format/fix | `npx eslint --ext .vue . --fix` |
| 项目级 gate | `npx --no-install eslint --ext .vue .` |
...
读取 `eslint.config.js`, `eslint.config.mjs`, `eslint.config.cjs`, `eslint.config.ts`, `.eslintrc.json`, `.eslintrc.js`, `.eslintrc.yml`, `.pre-commit-config.yaml`, `.eslintrc`, `.eslintrc.cjs` 及 CI 中的实际命令。仓库配置优先于本技能示例；如果两者冲突，先报告差异。
...
运行 `npx --no-install eslint --ext .vue .`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `npx eslint --ext .vue . --fix`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
1. **领域陷阱** — Vue SFC 需要匹配 Vue 版本的 parser/plugin；普通 ESLint 配置可能跳过 `<template>`。
