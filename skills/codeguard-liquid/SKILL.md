...
description: 使用 Codeguard 对 Liquid (Shopify) 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 theme-check . 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# Liquid (Shopify) Codeguard 门禁
...
- “检查这个 Liquid (Shopify) 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Liquid (Shopify) 的格式问题，语义问题先列清单。”
- “提交前验证 Liquid (Shopify) lint、测试和构建证据。”
...
1. 根据 `.liquid` 与项目标记识别 Liquid (Shopify) 工作区。
2. 执行 `theme-check .` 并保留退出码、作用域和工具版本。
...
- 用户要求检查或修复 Liquid (Shopify) 代码规范。
- AI 修改了 `.liquid`，需要提交前验证。
- CI 出现与 `theme-check .` 相关的失败。
...
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | gem install theme-check |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `theme-check .` |
| format/fix | `未接入` |
| 项目级 gate | `theme-check .` |
...
运行 `theme-check .`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
7. **领域陷阱** — Shopify Theme Check 面向 Shopify Liquid；其他 Liquid 方言需单独确认。
