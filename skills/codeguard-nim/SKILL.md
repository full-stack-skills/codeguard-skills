---
name: codeguard-nim
description: 使用 Codeguard 对 Nim 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 nim check src 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
1. 在仓库根确认 ``.nimble`` 或目标文件存在。
2. 探测工具：`nim --version`。
3. 先只读检查：`nim check src`。
4. 将结果分为：代码违规、配置缺失、工具缺失、工具内部错误。
5. 只对可安全修复项执行：`nimpretty -r .`。
6. 复跑 lint，并执行项目已有测试/构建门禁。

可直接提出：

- “检查这个 Nim 项目，先不要修改，给我 lint 失败分类。”
- “只修复 Nim 的格式问题，语义问题先列清单。”
- “提交前验证 Nim lint、测试和构建证据。”
...
1. 根据 `.nim` 与项目标记识别 Nim 工作区。
2. 执行 `nim check src` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `nimpretty -r .` 处理可逆问题并复扫。
...
| 项目标记 | `.nimble` |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | nimpretty 内置于 Nim |
| 探测命令 | `使用命令查找与项目配置检查` |
| lint | `nim check src` |
| format/fix | `nimpretty -r .` |
| 项目级 gate | `nim check src` |
...
运行 `nim check src`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `nimpretty -r .`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
1. **领域陷阱** — `nim check` 仍依赖项目 defines 与搜索路径；单文件默认上下文可能误报。
