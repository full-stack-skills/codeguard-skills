---
name: codeguard-cpp
description: 使用 Codeguard 对 C++ 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 clang-tidy --quiet '{file}' 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
...
3. 先只读检查：`bash -c 'find . \( -name '"'"'*.cpp'"'"' -o -name '"'"'*.hpp'"'"' -o -name '"'"'*.cc'"'"' \) -type f -print0 | xargs -0 -r clang-tidy --quiet'`。
...
- “检查这个 C++ 项目，先不要修改，给我 lint 失败分类。”
- “只修复 C++ 的格式问题，语义问题先列清单。”
- “提交前验证 C++ lint、测试和构建证据。”
...
| 项目级 gate | `bash -c 'find . \( -name '"'"'*.cpp'"'"' -o -name '"'"'*.hpp'"'"' -o -name '"'"'*.cc'"'"' \) -type f -print0 \| xargs -0 -r clang-tidy --quiet'` |
...
运行 `bash -c 'find . \( -name '"'"'*.cpp'"'"' -o -name '"'"'*.hpp'"'"' -o -name '"'"'*.cc'"'"' \) -type f -print0 | xargs -0 -r clang-tidy --quiet'`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
7. **领域陷阱** — 模板、宏和条件编译高度依赖真实编译数据库；不要凭单文件默认参数宣布通过。
