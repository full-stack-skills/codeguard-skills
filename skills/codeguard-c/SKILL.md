...
3. 先只读检查：`bash -c 'find . -name '"'"'*.c'"'"' -o -name '"'"'*.h'"'"' | xargs -r clang-tidy --quiet'`。
...
| 项目级 gate | `bash -c 'find . -name '"'"'*.c'"'"' -o -name '"'"'*.h'"'"' | xargs -r clang-tidy --quiet'` |
...
运行 `bash -c 'find . -name '"'"'*.c'"'"' -o -name '"'"'*.h'"'"' | xargs -r clang-tidy --quiet'`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
1. **领域陷阱** — clang-tidy 需要可靠的编译参数；复杂项目优先提供 `compile_commands.json`。
