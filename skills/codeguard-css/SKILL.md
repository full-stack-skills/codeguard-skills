---
name: codeguard-css
description: 使用 Codeguard 对 CSS / SCSS / Sass / LESS 项目执行并诊断代码规范门禁；当用户要求 lint、格式检查、自动修复、提交前质量验证，或出现 npx --no-install stylelint '**/*.css' 相关失败时使用。先确认仓库配置和工具可用性，区分通过、失败、无法验证与 planned，修复后必须复跑。
license: Apache-2.0
compatibility: 需要本地项目、对应语言工具链和仓库既有 lint 配置；默认只读检查，不自动安装依赖。
---

# CSS / SCSS / Sass / LESS Codeguard 门禁
...
2. 探测工具：`npx --no-install stylelint --version`。
3. 先只读检查：`npx --no-install stylelint '**/*.css'`。
4. 将结果分为：代码违规、配置缺失、工具缺失、工具内部错误。
5. 只对可安全修复项执行：`npx stylelint '**/*.css' --fix`。
6. 复跑 lint，并执行项目已有测试/构建门禁。

可直接提出：

- “检查这个 CSS / SCSS / Sass / LESS 项目，先不要修改，给我 lint 失败分类。”
- “只修复 CSS / SCSS / Sass / LESS 的格式问题，语义问题先列清单。”
- “提交前验证 CSS / SCSS / Sass / LESS lint、测试和构建证据。”
...
1. 根据 `.css`, `.scss`, `.sass`, `.less` 与项目标记识别 CSS / SCSS / Sass / LESS 工作区。
2. 执行 `npx --no-install stylelint '**/*.css'` 并保留退出码、作用域和工具版本。
3. 区分格式、静态规则、配置、依赖和环境类失败。
4. 使用 `npx stylelint '**/*.css' --fix` 处理可逆问题并复扫。
...
- 用户要求检查或修复 CSS / SCSS / Sass / LESS 代码规范。
- AI 修改了 `.css`, `.scss`, `.sass`, `.less`，需要提交前验证。
- CI 出现与 `npx --no-install stylelint '**/*.css'` 相关的失败。
...
| 项目标记 | 无专用标记文件 |
| 接入配置 | 以仓库现有配置为准 |
| 工具准备 | npm install --save-dev stylelint stylelint-config-standard |
| 探测命令 | `npx --no-install stylelint --version` |
| lint | `npx --no-install stylelint '**/*.css'` |
| format/fix | `npx stylelint '**/*.css' --fix` |
| 项目级 gate | `npx --no-install stylelint '**/*.css'` |
...
运行 `npx --no-install stylelint --version`。工具缺失、版本不兼容或配置无法加载均记为“无法验证”，不是通过。

### Step 4：执行只读检查

运行 `npx --no-install stylelint '**/*.css'`，记录命令、工作目录、退出码、工具版本和首个可操作错误。
...
优先运行 `npx stylelint '**/*.css' --fix`。只处理确定可逆的问题；依赖升级、规则豁免、生成文件和业务语义改动必须单独说明。
...
```text
CSS / SCSS / Sass / LESS Codeguard 结果
- 范围：<仓库/模块/文件>
- 工具：<名称与版本>
- 命令：<实际命令>
...
7. **领域陷阱** — Stylelint glob 必须覆盖 SCSS/Sass/LESS 时显式扩展，默认 `**/*.css` 不会自动覆盖全部预处理器。
